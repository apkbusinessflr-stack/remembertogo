...
export default function Map() {
  ...
  useEffect(() => {
    ...
    if (!key) { setError("Λείπει το NEXT_PUBLIC_MAPTILER_KEY (Vercel → Env Vars: Production)"); return; }

    const map = new maplibregl.Map({
      container: container.current,
      style: `https://api.maptiler.com/maps/streets-v2/style.json?key=${key}`,
      center: [23.7275, 37.9838], zoom: 6
    });
    ...
    map.on("load", async () => {
      try {
        ...
        map.addSource("places", { type: "geojson", data: { type: "FeatureCollection", features }, cluster: true, clusterRadius: 40 });
        ...
        map.on("moveend", async () => {
          ...
          const feats = j.items.map((p: any) => ({ type:"Feature", properties:{ id:p.id, title:p.title }, geometry:{ type:"Point", coordinates:[p.lng,p.lat] }}));
          const src = map.getSource("places") as maplibregl.GeoJSONSource;
          src.setData({ type: "FeatureCollection", features: feats });
        });
      } catch (err: any) { setError(`Map load failed: ${String(err?.message || err)}`); }
    });
  ...
  return (
    <div className="space-y-2">
      ...
      <div ref={container} className="w-full rounded-xl border" style={{ height: "70vh" }} />
    </div>
  );
}
