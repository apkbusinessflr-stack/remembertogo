...
export type PlaceRow = {
  id: string; owner: string; title: string; description: string | null; country_code: string | null;
  lat: number; lng: number; is_public: boolean; created_at: string;
};
export type ListPlacesParams = {
  country?: string | null; limit?: number; cursor?: string | null; bbox?: [number, number, number, number] | null;
};
function decodeCursor(cursor: string | null): { createdAt: string; id: string } | null {
  if (!cursor) return null;
  try { const [createdAt, id] = Buffer.from(cursor, "base64").toString("utf8").split("|"); if (!createdAt || !id) return null; return { createdAt, id }; }
  catch { return null; }
}
export async function listPublicPlaces(params: ListPlacesParams) {
  ...
  const minLng = params.bbox ? params.bbox[0] : null, minLat = params.bbox ? params.bbox[1] : null,
        maxLng = params.bbox ? params.bbox[2] : null, maxLat = params.bbox ? params.bbox[3] : null;
