...
/** ΑΥΤΑ είναι υποχρεωτικά σε Production */
type RequiredEnv = {
...
/** Αυτά είναι ΠΡΟΑΙΡΕΤΙΚΑ (μπορεί να είναι undefined) */
type OptionalEnv = {
...
/** Σε production απαιτούμε τιμές, σε preview/dev βάζουμε κενό/placeholder */
...
/** Απλά περνάμε ό,τι υπάρχει — ΜΠΟΡΕΙ να είναι undefined */
...
/** Βοηθητικό για λίστα admin emails */
export const adminEmails: string[] = (env.ADMIN_EMAILS || "")
  .split(",")
  .map((s) => s.trim().toLowerCase())
  .filter(Boolean);

/** Client-safe envs (μόνο public) */
